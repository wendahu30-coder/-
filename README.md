...
2021 年的时候，有学长在 GitHub 上发过一个校园网开机自动登录脚本，当时用起来挺方便的。后来今年学校校园网登录地址和接口更新了，原来的脚本就不能用了。所以我根据新版校园网登录流程重新适配了一版，主要是为了让大家连上 NIIT-WIFI 后不用每次手动点登录，开机/登录 Windows 后就能自动联网

## 快速使用

1. 下载本项目或 release 压缩包。
2. 解压到一个固定目录，不建议放在微信临时文件夹里。
3. 双击 `一键初始化.bat`(切记一定一定一定一定一定一定要断网状态包括vpn等全部关掉)。
5. 按提示输入校园网账号和密码。
6. 测试完成后，输入 `Y` 安装自动登录。
